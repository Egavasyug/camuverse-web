...
// Minimal ABI for OZ MinimalForwarder.execute
const forwarderAbi = [
  {
    type: 'function',
...
// Minimal ERC721 event to decode tokenId from receipt
const erc721Events = [
  {
    type: 'event',
    name: 'Transfer',
...
function toBigIntLike(v: string | number | bigint): bigint {
  ...
  // strings might be decimal or hex (0x...)
  const s = v.trim()
...
export async function POST(request: Request) {
  try {
    ...
    const FORWARDER_ADDRESS = process.env.FORWARDER_ADDRESS as `0x${string}` | undefined
    ...
    if (!FORWARDER_ADDRESS) {
      return NextResponse.json({ ok: false, error: 'FORWARDER_ADDRESS not set' }, { status: 500 })
    }
